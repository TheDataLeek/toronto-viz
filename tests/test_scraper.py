...
def test_db_write():
    ...
    vizlib.scraper.write_location_data(
...
def test_write_multiple_vehicles(conn):
    ...
    vizlib.scraper.write_location_data(
        _make_payload(vehicles), database_connection=conn
    )
...
def test_within_batch_dedup(conn):
    v = _make_vehicle("1")
    vizlib.scraper.write_location_data(_make_payload([v, v]), database_connection=conn)
    assert _row_count(conn) == 1
...
def test_cross_run_same_timestamp_not_reinserted(conn):
    ...
    vizlib.scraper.write_location_data(payload, database_connection=conn)
    vizlib.scraper.write_location_data(payload, database_connection=conn)
    assert _row_count(conn) == 1
